<?php
// Vérification que l'énigme existe et est de type audio
...
if (json_last_error() !== JSON_ERROR_NONE || !isset($donnees['question']) || !isset($donnees['audio_url']) || !isset($donnees['reponse_correcte'])) {
    echo '<div class="alert alert-danger">Données d\'énigme audio invalides</div>';
    return;
}
...
<div class='enigme-container'>
    <div class='question mb-4'>
        <h4 class='text-primary'> Question audio :</h4>
        <p class='lead'><?php echo htmlspecialchars($donnees['question']); ?></p>
    </div>
    
    <div class='audio-section mb-4'>
        <h5>🔊 Écoutez attentivement :</h5>
        <audio controls class='w-100'>
            <source src="<?php echo htmlspecialchars($donnees['audio_url']); ?>" type="audio/mpeg">
...
// Debug dans la console
console.log(' DEBUG TIMER INDICE AUDIO - Variables BDD:');
...
function validateAnswer() {
    ...
    validateAudioAnswer(reponseCorrecte, score);
}
...
function startIndiceTimer() {
    console.log('⏰ startIndiceTimer() AUDIO appelée avec timestamps BDD');
    ...
    console.log('  - Démarrage du timer d\'indice AUDIO avec BDD');
    ...
    const countdown = setInterval(() => {
        ...
        console.log('⏱️ Timer tick AUDIO BDD:', {
            now: now,
        ...
        if (remaining <= 0) {
            ...
            console.log('🎉 Indice AUDIO maintenant disponible !');
    ...
    console.log('  - Timer d\'indice AUDIO BDD démarré avec intervalle de 1 seconde');
}
