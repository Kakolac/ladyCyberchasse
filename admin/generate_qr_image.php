...
if (file_exists('../vendor/autoload.php')) {
    ...
    $totalHeight = imagesy($image) + 100; // +100 pour le texte
    ...
    imagestring($finalImage, 3, 10, $textY + 20, $lieuText, $black);
    
    // Token (tronqué pour la lisibilité)
    $tokenText = "Token: " . substr($token, 0, 8) . "...";
    imagestring($finalImage, 3, 10, $textY + 40, $tokenText, $black);
    ...
    if ($qrImageData !== false) {
        ...
        if ($qrImage !== false) {
            ...
            $this->createBasicQRCode($image, $qrUrl, $margin, $qrSize);
        }
    } else {
        // Fallback : créer un QR code basique
        $this->createBasicQRCode($image, $qrUrl, $margin, $qrSize);
    ...
    // Informations du lieu
    $lieuText = "Lieu: $lieu_nom (Ordre: $ordre)";
    imagestring($image, 3, $margin, $textY + 20, $lieuText, $black);
    
    // Token (tronqué pour la lisibilité)
    $tokenText = "Token: " . substr($token, 0, 8) . "...";
    imagestring($image, 3, $margin, $textY + 40, $tokenText, $black);
