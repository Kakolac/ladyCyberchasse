 ...
 * Version compatible avec les contraintes de clé étrangère
 */

session_start();
require_once '../config/connexion.php';

echo "<h1>�� Correction automatique des IDs des types d'énigmes (Production)</h1>";
...
try {
    ...
    // 3. Méthode alternative : mise à jour directe sans suppression de table
    ...
    // Créer une table temporaire avec un nom différent
    $pdo->exec("CREATE TABLE types_enigmes_new LIKE types_enigmes");
    echo "✅ Table temporaire créée<br>";
    
    // Insérer les types dans le bon ordre
    $stmt_insert = $pdo->prepare("INSERT INTO types_enigmes_new (id, nom, description, template, actif, created_at) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    $stmt = $pdo->query("SELECT id, template FROM types_enigmes_new ORDER BY id");
    $correspondance = [];
    ...
    // 5. Remplacer la table originale de manière sécurisée
    echo "<h3> Remplacement de la table originale...</h3>";
    
    // Renommer les tables
    $pdo->exec("RENAME TABLE types_enigmes TO types_enigmes_old");
    $pdo->exec("RENAME TABLE types_enigmes_new TO types_enigmes");
    echo "✅ Tables renommées<br>";
    
    // Réactiver la vérification des clés étrangères
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");
    echo "✅ Vérification des clés étrangères réactivée<br>";
    
    // Supprimer l'ancienne table (maintenant sans contrainte)
    $pdo->exec("DROP TABLE types_enigmes_old");
